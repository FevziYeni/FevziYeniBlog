...
export default function Header() {
 ...
 <a href="#anasayfa" className="flex items-center">
  ...
  />
      ...
      {menuOpen ? (
        <div className="fixed inset-0 z-50 bg-slate-950/95 p-5 text-white backdrop-blur-xl lg:hidden">
          ...
          <div className="mt-10 grid gap-4">
            {navItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={() => setMenuOpen(false)}
                className="rounded-2xl border border-white/10 bg-white/5 px-5 py-4 text-lg font-semibold"
              >
                {item.label}
              </a>
            ))}
